...
import { waitFor3DContent, takePageSnapshot, waitForPageLoad, VIEWPORTS } from './utils/test-helpers';
...
test.describe('Visual Snapshot Tests', () => {
  ...
  test.describe('Asset Pages', () => {
    test('should capture 3D objects asset page', async ({ page }) => {
      ...
      await page.waitForSelector('img, canvas', { timeout: 10000 }).catch(() => {});
  ...
  test.describe('Responsive Snapshots', () => {
    ...
    for (const pageInfo of pages) {
      test(`should capture ${pageInfo.name} page on tablet`, async ({ page }) => {
      ...
      test(`should capture ${pageInfo.name} page on desktop`, async ({ page }) => {
