...
describe("Storage Service", () => {
  ...
  describe("Given storage initialization", () => {
    it("When called, Then should create filesystem storage with userData path", () => {
      ...
      vi.mocked(createFileSystemStorage).mockReturnValue(
      ...
      expect(createFileSystemStorage).toHaveBeenCalledWith({
    ...
    it("When storage creation fails, Then should quit app gracefully", () => {
      ...
      vi.mocked(createFileSystemStorage).mockImplementation(() => {
        throw mockError;
      });
      const consoleSpy = vi
        .spyOn(console, "error")
