...
test.describe('Atomiton UI Design System - Visual Regression Tests', () => {
  ...
  test.describe('Desktop Viewport', () => {
    ...
    test('Homepage - Design System Overview', async ({ page }) => {
      ...
      await expect(page.locator('h1', { hasText: 'Atomiton UI Design System' })).toBeVisible();
    ...
    test('Button Component Page - All States and Variants', async ({ page }) => {
      ...
      await expect(page.locator('h3', { hasText: 'Primary Buttons' })).toBeVisible();
      await expect(page.locator('h3', { hasText: 'Secondary Buttons' })).toBeVisible();
      await expect(page.locator('h3', { hasText: 'Danger Buttons' })).toBeVisible();
      await expect(page.locator('h3', { hasText: 'Default Buttons' })).toBeVisible();
      await expect(page.locator('h3', { hasText: 'Size Variations' })).toBeVisible();
      await expect(page.locator('h3', { hasText: 'Full Width' })).toBeVisible();
      
      // Test specific button states
      await expect(page.locator('button', { hasText: 'Primary' }).first()).toBeVisible();
      await expect(page.locator('button:disabled').first()).toBeVisible();
      await expect(page.locator('button[data-loading="true"]').first()).toBeVisible();
    });
  ...
  test.describe('Navigation and Interaction Tests', () => {
    ...
    test('Active navigation state is correctly shown', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: 'Overview' })).not.toHaveAttribute('data-active', 'true');
      await expect(page.getByRole('link', { name: 'Cards' })).not.toHaveAttribute('data-active', 'true');
      await expect(page.getByRole('link', { name: 'Inputs' })).not.toHaveAttribute('data-active', 'true');
    });
  });
  ...
  test.describe('Theme Integration Tests', () => {
    ...
    test('Brainwave theme is properly applied', async ({ page }) => {
      ...
      const navbar = page.locator('[data-testid="navbar"], .mantine-AppShell-navbar').first();
