 ...
 * Shell Command node executable
 */
export const shellCommandExecutable: NodeExecutable<ShellCommandParameters> = createNodeExecutable({
  async execute(
    ...
    try {
      ...
      const command = getInputValue<string>(context, "command") || config.command;
      const inputArgs = getInputValue<string[] | string>(context, "args");
      const workingDirectory = getInputValue<string>(context, "workingDirectory") || config.workingDirectory;
      const inputEnvironment = getInputValue<Record<string, string>>(context, "environment");

      // Parse arguments
      let args: string[];
      if (inputArgs) {
        args = Array.isArray(inputArgs) ? inputArgs : parseJSON(inputArgs, config.args);
      } else {
        args = config.args;
      }
