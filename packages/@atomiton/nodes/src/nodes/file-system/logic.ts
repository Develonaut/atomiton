...
export const fileSystemLogic = createNodeLogic<FileSystemParameters>({
  async execute(
    context: NodeExecutionContext,
    config: FileSystemParameters,
  ): Promise<NodeExecutionResult> {
    try {
      const operation = getInputValue(context, "operation") || config.operation;
      const path = getInputValue(context, "path") || config.path;
      const content = getInputValue(context, "content") || config.content || "";
      ...
      if (!path) {
        ...
        return createErrorResult("Path is required for file system operations");
      }
