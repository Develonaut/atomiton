...
export function getNodeByType(type: string) {
  // Find node by its metadata.type field (e.g., "image-composite")
  return Object.values(nodes).find((node) => node.metadata.type === type);
}

export function getNodeTypes() {
  // Return actual type values from metadata (e.g., "image-composite")
  return Object.values(nodes).map((node) => node.metadata.type);
}
