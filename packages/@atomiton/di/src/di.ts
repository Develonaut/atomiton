/**
 * @atomiton/di - Lightweight Dependency Injection for Atomiton
 *
 * Based on @n8n/di (https://github.com/n8n-io/n8n/tree/master/packages/%40n8n/di)
 * Licensed under the Fair-code license
 *
 * This implementation is adapted from n8n's DI package with modifications for Atomiton's needs.
 * Original implementation by n8n team, adapted by Atomiton team.
 */
...
/**
 * Alias for Service decorator for those who prefer @Injectable
 * @deprecated Use @Service instead for consistency
 */
...
class ContainerClass {
  ...
  get<T>(type: ServiceIdentifier<T>): T {
    ...
    try {
      ...
      const dependencies = paramTypes.map(
        <P>(paramType: Constructable<P>, index: number) => {
      ...
      if (
        ...
        throw new DIError(
          `${type.name} is an abstract class, and cannot be instantiated`,
        );
      }
      throw error;
    } finally {
   ...
   * @example
   * ```typescript
   * Container.set(AbstractService, new ConcreteService());
   * ```
   */
  ...
  /**
   * Completely clears the container, removing all registrations and instances
   * Use with caution - this removes all service registrations
   */
  clear(): void {
    instances.clear();
  }
 ...
 * @example
 * ```typescript
 * import { Container, Service } from '@atomiton/di';
 *
 * @Service()
 * class MyService {}
 *
